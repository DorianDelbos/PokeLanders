...
namespace Landers
{
	...
	public class LanderData
	{
		...
		public struct Stats
		{
			...
			public Stats(List<BaseStats> stats)
			{
		...
		// Levels
		public byte Level => LanderUtilities.GetLevelByXp(Xp, BaseXp);
		public int Xp { get => statsData.xp; private set => statsData.xp = value; }
		// States
		public ushort Hp { get => statsData.hp; set => statsData.hp = value; }
		public ushort MaxHp => LanderUtilities.GetMaxHp(statsData.stats.hp, Level, statsData.ivs.hp, statsData.evs.hp);
		public ushort Attack => LanderUtilities.GetStatValue(statsData.stats.attack, Level, statsData.ivs.attack, statsData.evs.attack, NatureRepository.GetByName(Nature).GetStatMultiplier("Attack"));
		public ushort SpecialAttack => LanderUtilities.GetStatValue(statsData.stats.specialAttack, Level, statsData.ivs.specialAttack, statsData.evs.specialAttack, NatureRepository.GetByName(Nature).GetStatMultiplier("Special-Attack"));
		public ushort Defense => LanderUtilities.GetStatValue(statsData.stats.defense, Level, statsData.ivs.defense, statsData.evs.defense, NatureRepository.GetByName(Nature).GetStatMultiplier("Defense"));
		public ushort SpecialDefense => LanderUtilities.GetStatValue(statsData.stats.specialDefense, Level, statsData.ivs.specialDefense, statsData.evs.specialDefense, NatureRepository.GetByName(Nature).GetStatMultiplier("Special-Defense"));
		public ushort Speed => LanderUtilities.GetStatValue(statsData.stats.speed, Level, statsData.ivs.speed, statsData.evs.speed, NatureRepository.GetByName(Nature).GetStatMultiplier("Speed"));
		...
		#region CONSTRUCTORS
		public LanderData(LanderDataNFC nfcData, Lander landerModel)
		...
		public LanderData(string tag, ushort id, string species, string name, string description, int xp, ushort hp, byte happiness, string nature, byte baseHp, byte attack, byte specialAttack, byte defense, byte specialDefense, byte speed, byte ivPv, byte ivAtk, byte ivAtkSpe, byte ivDef, byte ivDefSpe, byte ivSpeed, byte evPv, byte evAtk, byte evAtkSpe, byte evDef, byte evDefSpe, byte evSpeed, ushort attack1, ushort attack2, ushort attack3, ushort attack4, ushort baseXp, ushort height, ushort weight, bool isMale, bool isShiny, List<string> types, string modelUrl)
        ...
        public LanderData(string tag, ushort id, string species, string name, string description, int xp, ushort hp, byte happiness, string nature, Stats stats, Stats ivs, Stats evs, ushort attack1, ushort attack2, ushort attack3, ushort attack4, ushort baseXp, ushort height, ushort weight, bool isMale, bool isShiny, List<string> types, string modelUrl)
			: this(new MainData(tag, id, species, name, description),
				   new StatsData(xp, hp, happiness, nature, stats, ivs, evs),
				   new MovesData(attack1, attack2, attack3, attack4),
				   new OtherData(baseXp, height, weight, isMale, isShiny, types, modelUrl)) { }

		public LanderData(MainData mainData, StatsData statsData, MovesData attacksData, OtherData otherData)
		{
