...
namespace Landopedia
{
    public class GameManager : MonoBehaviour
    {
        ...
        private Lander[] landers;

        public Lander[] Landers => landers;
        ...
        {
            landers = await DataFetcher<Lander>.FetchArrayDataAsync("api/v1/lander", OnSuccess, ExeptionError);
            ...
            foreach (Lander lander in landers)
            {
                ...
                landerCaseInstance.Initialize(lander.id, await WebSpriteLoader.LoadSpriteFromUrlAsync(lander.sprite));
                landerCaseInstance.SetHasLander(landerIds.Contains(lander.id));
            }
        }

        private void OnSuccess(Lander[] landers)
        {
